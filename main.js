...
const addQuizButton = async () => {
    log('Attempting to add Quiz button...');
    const buttonContainerSelector = '.quiz__header';
    const container = await waitForElements(buttonContainerSelector, 5000, true);

    if (container) {
        if (!container.querySelector('#auto-quiz-start-btn')) {
            log('Quiz header container found, adding button.');
            ...
            quizButton.addEventListener('click', async () => {
                ...
                if (!csrfToken) { alert('CSRF токен не найден.'); logError('Quiz start blocked: CSRF token is null or empty.'); return; }

                quizButton.disabled = true;
                quizButton.textContent = '⏳ Квиз запущен...';
                log('Запускаем startQuiz...');
                try {
                    await startQuiz();
                    quizButton.textContent = '✔️ Квиз завершен';
                    log('startQuiz завершил работу.');
                } catch (error) {
                    logError('Ошибка во время выполнения startQuiz:', error);
            ...
            log('Авто-Квиз button added.');
        } else {
            log('Авто-Квиз button already exists.');
        }
    } else {
         logWarn(`Quiz header container ('${buttonContainerSelector}') not found after waiting.`);
    }
};
...
const initPage = async () => {
    ...
    if (context === 'quizPage') {
         log('Context is quizPage, calling addQuizButton...');
         await addQuizButton();
    } else if (context === 'minePage') {
         log('Context is minePage, calling initMinePage...');
         await initMinePage();
    }
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === 'clearWishlistCache') {
        ...
        getSettings().then(settings => {
            if (settings.extensionEnabled) {
                const context = getCurrentContext();
                if (context && contextsSelectors[context] && context !== 'quizPage' && context !== 'minePage') {
                   const oldLabels = document.querySelectorAll('.wishlist-warning, .owners-count');
                   oldLabels.forEach(label => label.remove());
